# ProofWeave user guide

[English landing page](../README.md) | [繁體中文](../README.zh-TW.md) | [简体中文](../README.zh-CN.md) | [日本語](../README.ja.md)
...
| Validation | Versioned JSON Schema Draft 2020-12 contracts for facts, sources, experiments, models, routing, and reviews |
...
| Agent layer | Canonical roles, workflows, prompts, and generated Codex/Claude adapters |
...
| Release gate | Runs automated tests plus structure, schema, graph, source, experiment, paper, secret, and optional PDF checks |
...
- canonical roles, workflows, prompts, schemas, and native Codex/Claude adapters that point back to the canonical files;
- an isolated odd-sum proof workflow and dry routing demonstrations;
...
On Windows, if the bare `python` alias does not resolve to Python 3.11 or newer,
use the Python launcher with an installed version:

```powershell
py -3.11 -m mathlab status
py -3.11 -m unittest discover -s tests -v
py -3.11 -m mathlab release-check
```
...
See [`operator_guide.md`](operator_guide.md), [`workflow_guide.md`](workflow_guide.md), and [`design/architecture.md`](design/architecture.md).
