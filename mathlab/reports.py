...
SUPPORTED_STATUS_LOCALES = {"en", "zh-TW"}
...
def render_status_markdown(report: dict[str, Any], locale: str) -> str:
    ...
    if locale not in SUPPORTED_STATUS_LOCALES:
        raise ValueError(f"Unsupported status locale: {locale}")
    english = locale == "en"
...
def write_status_sources(root: Path | str, output_directory: Path | str | None = None) -> dict[str, Path]:
    ...
    for locale in ("en", "zh-TW"):
        ...
        templates[locale] = template_path.read_text(encoding="utf-8")

    rendered_markdown = {
        locale: render_status_markdown(report, locale) for locale in ("en", "zh-TW")
    }
    rendered_tex = {
        locale: render_status_tex(report, locale, templates[locale])
        for locale in ("en", "zh-TW")
    }
    ...
    for locale in ("en", "zh-TW"):
        ...
        atomic_write_text(markdown_path, rendered_markdown[locale])
        atomic_write_text(tex_path, rendered_tex[locale])
