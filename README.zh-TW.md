...
> **專案狀態：**實驗性研究基礎設施。已發布的 repository/evidence release
> 是 `v0.1.0`；Core runtime/package 與 protocol version 是 `2.0.0`。兩者是不同版本軸。

**已核實的 `v0.1.0` evidence snapshot：**[公開 release](https://github.com/f0909172434/proofweave-math-lab/releases/tag/v0.1.0)
記錄 fixed corpus 42/42 通過，其中 positive cases 14/14、14 個成對 negative
cases 的 false certifications 為 0，且 8 個 escape attacks 的接受數為 0。
[該 tag 的 CI](https://github.com/f0909172434/proofweave-math-lab/actions/runs/31348288988)
執行 79 個 fast tests，branch-aware coverage 為 96%，其中有 2 個 Lean-dependent
skips；真實 Lean jobs 在 Ubuntu 與 Windows 皆執行完整 79 個測試，coverage
為 97%，permitted skips 為 0，並使用固定的 Lean/Mathlib 4.32.2。

這些是有限語料的 `COMPUTATIONAL` 結果，不足以建立全域 soundness、
completeness、無缺陷、自然語言與 formal target 的語意等價，或精確受認證
target 以外的 theorem truth。
